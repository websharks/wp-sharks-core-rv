...
{
    return !___wp_sharks_core_rv_issue();
}
...
function ___wp_sharks_core_rv_issue(): array
{
    ...
    $cur_version = ___wp_sharks_core_rv_get_version();

    if (!$cur_version) { // Inactive or missing?
    ...
    } elseif ($min_version && version_compare($cur_version, $min_version, '<')) {
        return ['reason' => 'needs-upgrade', 'cap' => 'update_plugins'];
    } elseif ($max_version && version_compare($cur_version, $max_version, '>')) {
...
function ___wp_sharks_core_rv_initialize()
{
    ...
    $___wp_sharks_core_rv = [ // Internal use only.
        'min' => '', // Minimum required version string.
        'max' => ''// Maximum compatible version string.
    ];
